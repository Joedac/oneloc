<template>
  <div class="container is-fluid" style="margin-top: 50px" v-if="total > 0">
    <h2 class="title has-text-primary is-3 has-text-weight-medium has-text-centered">
      {{ welcome }}
      <i class="fas fa-code"></i> !
    </h2>
    <h3 class="has-text-centered has-text-primary">{{ subtitle }}</h3>
    <br />
...
export default {
  ...
  mounted() {
    ...
    self.$store.dispatch("getFunctionsStore");
    self.$store.dispatch("getCategoriesStore");
  },
